...
namespace Helloween
{
    public class Monster : Entity
    {
        ...
        public string Meet(Human meet)
        {
            ...
            builder.Append($"Монстр:{name}[ID:{ID}] встречает челика [ID:{meet.ID}] и ");
            lifespan -= 1;
            if (rnd.Next(0, 11) < 3)
            {
                //gives
            ...
            else
            {
                ...
                if (meet.witcher)
                {
                    ...
                    builder.Append($"он оказывается ведьмаком. Помянем...");
                    //kills monster
                }
                else
                {
                    ...
                    {
                        builder.Append($"ничего не может сделать, так как у челика недостаточно {needs}");
                    }
                }
            }

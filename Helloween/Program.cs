...
namespace Helloween
{
    class Program
    {
        static void Main()
        {
            ...
            while (monsters.Count > 0)
            {
                foreach (var mnst in monsters)
                {
                    ...
                    else if (mnst.inventory[ItemType.Life] == 0 || mnst.lifespan == 0)
                    {
                        ...
                        {
                            Console.WriteLine($"Монстр [ID:{mnst.ID}] сдох от рук ведьмака");
                        }
                        else
                        {
                            Console.WriteLine($"Монстр [ID:{mnst.ID}] сдох от старости");
                        }
                        mnstDieCount++;
                    ...
                    if (crowd[index].inventory[ItemType.Life] <= 0)
                    {
                        if (crowd[index].inventory[ItemType.Immortality] >= 1)
                        {
                            ...
                            Console.WriteLine($"Челик [ID:{crowd[index].ID}] бессметрный, поэтому сдохнуть не может");
                        }
                        else
                        {
                            Console.WriteLine($"Челик [ID:{crowd[index].ID}] сдох");
                            humanDieCount++;
            ...
            Console.WriteLine($"Монстров наелось и спят: {mnstSleepCount}");
            Console.WriteLine($"Монстров сдохло: {mnstDieCount}");
            Console.WriteLine($"Челиков сдохло: {humanDieCount}");
            Console.ReadKey();
        }
